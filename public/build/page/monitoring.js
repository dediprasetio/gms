...
function highchartLineAmp(dataR, dataS, dataT){
...
function updateChartAmphereR(valueData, formatterLabel, curentData) {
    if (curentData != valueData) {
        ...
        optionAmphereR.series[0].detail.formatter = valueData + ' ' + formatterLabel;
...
function updateChartAmphereS(valueData, formatterLabel, curentData) {
    if (curentData != valueData) {
        ...
        optionAmphereS.series[0].detail.formatter = valueData + ' ' + formatterLabel;
...
function updateChartAmphereT(valueData, formatterLabel, curentData) {
    if (curentData != valueData) {
        ...
        optionAmphereT.series[0].detail.formatter = valueData + ' ' + formatterLabel;
...
function updateChartVoltageR(valueData, formatterLabel, curentData) {
    if (curentData != valueData) {
        ...
        optionVoltagerR.series[0].detail.formatter = valueData + ' ' + formatterLabel;
...
function updateChartVoltageS(valueData, formatterLabel, curentData) {
    if (curentData != valueData) {
        ...
        optionVoltageS.series[0].detail.formatter = valueData + ' ' + formatterLabel;
...
function updateChartVoltageT(valueData, formatterLabel, curentData) {
    if (curentData != valueData) {
        ...
        optionVoltageT.series[0].detail.formatter = valueData + ' ' + formatterLabel;
...
function getPowerData(timestamp) {
    //  var queryString = {'timestamp' : timestamp};
    axios.get(
        base_url + 'api/power/all-power', {
        headers: {
        ...
        var ampPhaseR = [];
        $.each( rowData.PHASE_R, function( key, value ) {
            ampPhaseR.push({
                x: value.labels,
                y: value.Current
              });
        ...
        **********************************************************************************************************************************/
        try {
            ...
            highchartLineAmp(ampPhaseR, ampPhaseS, ampPhaseT);
        ...
        **********************************************************************************************************************************/
        try {
            ...
            //USAGE & STATUS
            $('#VoltUsageR').html(percentageVoltageUsage(powerData.PHASE_R.Voltage));
            $('#VoltUsageS').html(percentageVoltageUsage(powerData.PHASE_S.Voltage));
            $('#VoltUsageT').html(percentageVoltageUsage(powerData.PHASE_T.Voltage));
        ...
        setTimeout(function () {
            getPowerData(res.data.effective_date);
        }, timeoutFunction);
        ...
        const resp = axios.get(
            ...
            response.forEach(function callback(element, index, array) {
                if (initializeChillerStatus == false) {
                    initializeChiller(element.uniq_id, element.name, element.tool_brand, element.value);
                } else {
        ...
        const resp = axios.get(
            ...
            response.forEach(function callback(element, index, array) {
                if (initializeFreezerStatus == false) {
                    initializeFreezer(element.uniq_id, element.name, element.tool_brand, element.value);
                } else {
                    updateToolStatus(element.uniq_id, element.value);
                }
            });
        ...
        const resp = axios.get(
            ...
            response.forEach(function callback(element, index, array) {
                if (initializeTemperatureStatus == false) {
                    initializeTemperature(element.uniq_id, element.name, element.tool_brand, element.value);
                } else {
...
function init_switch() {
    if ($(".switch-cashier-1")[0]) {
        var elems = Array.prototype.slice.call(document.querySelectorAll('.switch-cashier-1'));
...
function initializeChiller(uniqId, name, brand, value) {
    var status = '<p class="mt-2 text-info">Status : <span class="badge badge-success "> ON </span></p>';
    var valueId = `value-${uniqId}`;
    if (value == 0) {
        status = '<p class="mt-2 text-danger">Status : <span class="badge badge-danger"> OFF </span></p>';
    }
    $('#card-chiller')
        .append(`<div class="col-lg-3 col-md-12 col-sm-12">
                <div class="x_panel">
                    <div class="row">
...
function initializeTemperature(uniqId, name, brand, value) {
    ...
    $('#card-temperature').append(`<div class="col-lg-4 col-md-12 col-sm-12  ">
        <div class="x_panel">
            <div class="row">
                <div class="col-lg-12 col-md-3 col-sm-6 mt-2">
                    <div class="card text-center p-1 border-none-mobile">
